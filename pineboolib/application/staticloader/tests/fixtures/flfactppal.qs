...
class oficial extends interna {
	...
	function ejercicioActual2():String {
		return this.ctx.oficial_ejercicioActual2();
	}
  ...
  function  escapeQuote(str)  { return this.ctx.oficial_escapeQuote(str); }
    ...
    function dameColor(nombre)
    {
 	...
 	function dameFechaActual() {
		return this.ctx.oficial_dameFechaActual();
	}
	function dameTiempoActual() {
		return this.ctx.oficial_dameTiempoActual();
	}
	function dameHoraActual() {
		return this.ctx.oficial_dameHoraActual();
	}
	function abreCxTx() {
...
class ifaceCtx extends head {
      ...
      function pub_valorDefecto(fN)
  {
	...
	function pub_dameFechaActual() {
		return this.dameFechaActual();
	}
	function pub_dameTiempoActual() {
		return this.dameTiempoActual();
	}
	function pub_dameHoraActual() {
		return this.dameHoraActual();
	}
}
...
function oficial_msgNoDisponible(nombreModulo:String)
{
	...
	MessageBox.information(util.translate("scripts", "El m�dulo '") +
		...
		"' no est� disponible."), MessageBox.Ok, MessageBox.NoButton, MessageBox.NoButton);
}
...
\end */
function oficial_ejercicioActual2():String
{
	var util:FLUtil = new FLUtil;
...
/** Indica si la extensi�n est� cargada o no en el sistema. Funci�n a sobrecargar por cada extensi�n devolviendo true en caso de que el nE coincida con el de la extensi�n.
*/
...
function oficial_calcularIdentificadorAcreedor(cifEmpresa, codCuenta)
{
    ...
    if (!codPais) {
        sys.warnMsgBox(sys.translate("No se ha podido obtener el c�digo ISO del pa�s asociado a la empresa"));
        return false;
    }
...
function oficial_dameFechaActual()
{
	var _i = this.iface;
	_i.abreCxTx();

	var f = AQUtil.quickSqlSelect("empresa", "CURRENT_DATE", "1 = 1", _i.cxTx_);
	return f.toString().left(10);
}

function oficial_dameTiempoActual()
{
	var _i = this.iface;
	_i.abreCxTx();

	var t = AQUtil.quickSqlSelect("empresa", "CURRENT_TIME", "1 = 1", _i.cxTx_);
	return t.toString();
}

function oficial_dameHoraActual()
{
	var _i = this.iface;
	var t = _i.dameTiempoActual();
...
function oficial_abreCxTx()
{
	...
	if (dbTx.connectionName() != _i.cxTx_ || !dbTx.isOpen()) {
		if (!sys.addDatabase(_i.cxTx_)) {
			_i.ponMsgError(sys.translate("Error al establecer la conexi�n de comprobaci�n de transacciones."), "warn", this);
			return false;
		}
	}
	return true;
