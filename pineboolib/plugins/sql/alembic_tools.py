...
class Migration:
    ...
    def __init__(self, conn: "iconnection.IConnection", metadata: "itablemetadata.ITableMetaData"):
        """Initialize."""

        self._conn = conn

        self._table_name = metadata.name()
    ...
    def upgrade(self) -> bool:
        ...
        print("Paso 1")
        config.main(argv=["revision", "-m", "'%s'" % (self._table_name)])
        print("Paso 2")
    ...
    def exists_migrations(self):
        """Check if migrations exists"""

        return os.path.exists(os.path.join(self._alembic_folder, "env.py"))
